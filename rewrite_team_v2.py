...
file_path = "apps/mobile/app/(buyer)/team.tsx"
...
type RoleKey = 'OWNER' | 'MANAGER' | 'MEMBER' | 'DRIVER';
...
const s = {
  ...
  // Actions
  primaryBtn: "bg-[#111827] rounded-xl py-4 items-center mt-6 shadow-sm",
  primaryBtnText: "text-white font-bold text-[16px]",
  deleteBtn: "flex-row items-center justify-center gap-2 mt-6 py-4 bg-[#fee2e2] rounded-xl",
...
function MemberDetailsSheet({
  ...
  const isSelf = false; // logic handled in parent usually, or user object
  const isOwner = member.companyRole === 'OWNER';
  const canEdit = currentUserRole === 'OWNER' && !isOwner; // Owners can edit others, but not other owners easily here
    ...
    >
      <View className={s.sheetSection}>
        ...
        <View className="mb-6 flex-row gap-4">
          <View className="flex-1 bg-[#f9fafb] p-3 rounded-lg">
            <Text className="text-xs text-gray-500 font-medium mb-1">TELEFONS</Text>
            <Text className="text-sm font-semibold text-gray-900">{member.phone || '—'}</Text>
          </View>
          <View className="flex-1 bg-[#f9fafb] p-3 rounded-lg">
            <Text className="text-xs text-gray-500 font-medium mb-1">LOMA</Text>
...
export default function TeamScreen() {
  ...
  async function handleInvite(data: InviteMemberInput) {
    ...
    try {
      ...
      // If it returns a member object (already joined vs invited) depends on API, 
      // typically returns the created record
