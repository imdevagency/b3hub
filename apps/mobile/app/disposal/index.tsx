...
const WASTE_OPTIONS: WasteOption[] = [
  ...
  { id: 'HAZARDOUS', label: 'Bīstami atkritumi', desc: 'Azbests, krāsas, šķīdinātāji', Icon: AlertTriangle },
];
...
export default function DisposalWizard() {
  ...
  const [picked, setPicked] = useState<PickedAddress | null>(
    state.locationLat != null && state.locationLng != null && state.location
      ? { address: state.location, lat: state.locationLat, lng: state.locationLng, city: state.locationCity ?? '' }
      : null,
  );
  ...
  }, [state, preset, truck, desc, date, token, contactName, contactPhone, notes, setTruckType, setTruckCount, setDescription, setRequestedDate, reset]);
  ...
  const ctaLabel = step === 4 ? `Pasūtīt — no €${preset.fromPrice * preset.truckCount}` : 'Turpināt';

  const onCTA = useCallback(() => {
    if (step === 1) { setShowPicker(true); return; }
            ...
            <View style={s.wasteGrid}>
              {WASTE_OPTIONS.map((opt) => {
                ...
                return (
                  ...
                  >
                    {isSel && (
                      <View style={s.checkBadge}><Check size={11} color="#fff" /></View>
                    )}
          ...
          <ScrollView style={s.content} contentContainerStyle={s.pad} showsVerticalScrollIndicator={false}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 20 }}>
              {Array.from({ length: 14 }, (_, i) => {
                ...
                return (
                  <TouchableOpacity key={i} style={[s.dayChip, isSel && s.dayChipActive]} onPress={() => setDate(d)} activeOpacity={0.75}>
                    <Text style={[s.dayDow, isSel && s.dayActive]}>{d.toLocaleDateString('lv-LV', { weekday: 'short' })}</Text>
                    <Text style={[s.dayNum, isSel && s.dayActive]}>{d.getDate()}</Text>
                    <Text style={[s.dayMon, isSel && s.dayActiveSub]}>{d.toLocaleDateString('lv-LV', { month: 'short' })}</Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
            ...
            <View style={s.summaryCard}>
              ...
              <SumRow icon="♻️" label="Atkritumu veids" value={selectedWaste ? WASTE_LABELS[selectedWaste] : '—'} />
              <SumRow icon="🚛" label="Transports" value={`${preset.truckCount} × ${truck.label}`} />
              <SumRow icon="📦" label="Apjoms" value={`${truck.capacity * preset.truckCount} t ≈ ${truck.volume * preset.truckCount} m³`} />
              <SumRow icon="💰" label="Orientējošā cena" value={`no €${preset.fromPrice * preset.truckCount} + PVN 21%`} />
            </View>
...
const s = StyleSheet.create({
  ...
  wasteCard: {
    ...
    borderWidth: 1.5, borderColor: '#e5e7eb', padding: 14,
  ...
  // Hazard
  hazardRow: { flexDirection: 'row', gap: 8, alignItems: 'center', backgroundColor: '#fef2f2', borderRadius: 8, padding: 12, marginBottom: 16 },
  ...
  successSub: { fontSize: 14, color: '#6b7280', marginBottom: 24, textAlign: 'center', lineHeight: 20 },
