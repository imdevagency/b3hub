...
export default function DisposalStep3Location() {
  const router = useRouter();
  const { state, setLocation } = useDisposal();
    ...
    <ScreenContainer standalone bg="#fff">
      ...
      >
        ...
        <View style={s.body}>
          ...
          <Text style={s.stepTitle}>Kur atrodas atkritumi?</Text>
          <Text style={s.stepSubtitle}>
            Norādiet būvlaukuma vai objekta adresi, kur jāatbrauc pēc atkritumiem.
          </Text>
          ...
          >
            ...
            <Text style={[s.locationText, !isValid && s.locationPlaceholder]} numberOfLines={2}>
              {isValid ? state.location : 'Meklēt adresi...'}
            </Text>
        ...
        <View style={s.footer}>
          <TouchableOpacity
            ...
            disabled={!isValid}
            onPress={() => router.push('/disposal/quantity')}
            activeOpacity={0.8}
          >
      ...
      <AddressPicker
        visible={pickerOpen}
        title="Atkritumu atrašanās vieta"
        ...
        onConfirm={({ address, lat, lng }) => {
          ...
          setLocation(address, cityPart, lat, lng);
          setPickerOpen(false);
        }}
