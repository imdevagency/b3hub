...
export default function TransportStep2Dropoff() {
  const router = useRouter();
  const { state, setDropoff } = useTransport();
  const [pickerOpen, setPickerOpen] = useState(false);
  const isValid = state.dropoffAddress.trim().length >= 3;
    ...
    <ScreenContainer standalone bg="#fff">
      ...
      >
        ...
        <View style={s.progressWrap}>
          ...
          <Text style={s.progressLabel}>Solis 2 / 4</Text>
        </View>

        <View style={s.body}>
          <Text style={s.stepNum}>02</Text>
          <Text style={s.stepTitle}>Kur izkraut?</Text>
          <Text style={s.stepSubtitle}>Norādiet izkraušanas adresi — kur jāpiegādā krava.</Text>
          ...
          >
            ...
            <Text style={[s.locationText, !isValid && s.locationPlaceholder]} numberOfLines={2}>
              {isValid ? state.dropoffAddress : 'Meklēt izkraušanas adresi...'}
            </Text>
            <ChevronRight size={16} color="#9ca3af" />
          </TouchableOpacity>

          {isValid && state.dropoffLat != null && (
            <Text style={s.coordsHint}>
              {state.dropoffLat.toFixed(5)}, {state.dropoffLng?.toFixed(5)}
            </Text>
          )}
        </View>
        ...
        <View style={s.footer}>
          <TouchableOpacity
            ...
            disabled={!isValid}
            onPress={() => router.push('/transport/vehicle')}
            activeOpacity={0.8}
          >
      ...
      <AddressPicker
        ...
        initialLat={state.dropoffLat ?? undefined}
        initialLng={state.dropoffLng ?? undefined}
        pinColor="#111827"
        onConfirm={({ address, lat, lng }) => {
          const cityPart = address.split(',').slice(-2, -1)[0]?.trim() ?? '';
          setDropoff(address, cityPart, lat, lng);
          setPickerOpen(false);
        }}
