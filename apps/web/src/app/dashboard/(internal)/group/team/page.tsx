/**
 * Grupas komanda — /dashboard/group/team
 *
 * All field workers across B3 Construction, B3 Hub deliveries, and
 * B3 Recycling in one roster. Same backend API as the construction employees
 * page (adminGetEmployees) — the dataset is shared, only the context changes.
 *
 * TODO: Add a `buContext` field to the ConstructionEmployee model so each
 * worker can be tagged with their primary BU, then surface that as a filter.
 */
'use client';
...
export default function GroupTeamPage() {
  ...
  async function handleSave() {
    ...
    setFormError(null);
    try {
      const payload = { ...form, defaultRateEntryId: form.defaultRateEntryId || undefined };
      if (editEmployee) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Komanda"
        description={`${activeCount} aktīvi darbinieki — šoferi, operatori un strādnieki visos 3 biznesa virzienos.`}
        action={
