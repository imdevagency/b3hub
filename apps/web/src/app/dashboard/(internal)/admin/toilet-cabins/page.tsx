/**
 * Admin toilet cabin orders page — /dashboard/admin/toilet-cabins
 * Platform-wide view of all ToiletCabinOrder records with status filtering.
 */
'use client';
...
import { adminGetToiletCabinOrders, type AdminToiletCabinOrder } from '@/lib/api/admin';
import { PageHeader } from '@/components/ui/page-header';
import { EmptyState } from '@/components/ui/empty-state';
import { Button } from '@/components/ui/button';
import { RefreshCw, Building2, Search } from 'lucide-react';
...
export default function AdminToiletCabinsPage() {
  const { user, token, isLoading } = useAuth();
  const router = useRouter();
  const [orders, setOrders] = useState<AdminToiletCabinOrder[]>([]);
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await adminGetToiletCabinOrders(token);
      setOrders(data);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Tualetes kabīņu noma"
        description={`${orders.length} kabīņu nomas pasūtījumi platformā`}
        action={
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon={Building2}
          title="Nav tualetes kabīņu pasūtījumu"
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((o) => (
                  <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-700 max-w-36 truncate">
                      <p>{o.address}</p>
