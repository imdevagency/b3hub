...
const MATERIAL_LABELS: Record<string, string> = {
  ...
  CONCRETE: 'Betons',
  SOIL: 'Grunts',
  RECYCLED_CONCRETE: 'RC betons',
  RECYCLED_SOIL: 'RC grunts',
  ASPHALT: 'Asfalta',
  CLAY: 'Māls',
  OTHER: 'Cits',
};
...
export default function ProjectDetailPage() {
  ...
  const router = useRouter();
  const { token } = useAuth();
  const { categoryLabels: MATERIAL_LABELS } = useMaterialCatalogue();
  ...
  const handleAddWasteDeclaration = async () => {
    ...
    setSavingWaste(true);
    try {
      const decl = await addWasteDeclaration(
        id,
        {
          wasteType: wType,
  ...
  const handleAddMaterialNeed = async () => {
    ...
    setSavingNeed(true);
    try {
      const need = await addMaterialNeed(
        id,
        {
          ...
          {wasteFormOpen && (
            <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
              ...
              <div className="space-y-1">
                ...
                <Input
                  className="h-9 text-sm"
                  placeholder="Papildinformācija par materiālu stāvokli..."
                  value={wNotes}
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="checkbox"
                  checked={wSell}
                  onChange={(e) => setWSell(e.target.checked)}
                  className="rounded"
                />
                Vēlamies pārdot (pēc apstrādes tiks piedāvāts Bilt tirgū)
              </label>
              <div className="flex gap-2 justify-end">
                <Button size="sm" variant="outline" onClick={() => setWasteFormOpen(false)}>
                  Atcelt
                </Button>
                <Button
                  size="sm"
                  disabled={!wTonnes || !wFrom || !wTo || savingWaste}
                  onClick={handleAddWasteDeclaration}
                >
          ...
          {wasteDeclarations.length === 0 && !wasteFormOpen ? (
            ...
            wasteDeclarations.map((d) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold">
                      {WASTE_LABELS[d.wasteType] ?? d.wasteType}
                    </p>
                    ...
                    {d.willingToSell && (
                      <Badge
                        variant="outline"
                        className="text-[10px] h-4 px-1.5 rounded-full border-green-500 text-green-700"
                      >
                        Pārdot
                      </Badge>
                    )}
                ...
                <button
                  onClick={() => handleDeleteWasteDeclaration(d.id)}
          ...
          {needFormOpen && (
            <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    placeholder="0"
                    value={nTonnes}
                    onChange={(e) => setNTonnes(e.target.value)}
                    type="number"
                    min="0.1"
                  />
                </div>
              ...
              <div className="space-y-1">
                ...
                <Input
                  className="h-9 text-sm"
                  placeholder="Specifikācija, kvalitāte, piegādes adrese..."
                  value={nNotes}
                  onChange={(e) => setNNotes(e.target.value)}
                />
              </div>
              <div className="flex gap-2 justify-end">
                <Button size="sm" variant="outline" onClick={() => setNeedFormOpen(false)}>
                  Atcelt
                </Button>
                <Button
                  size="sm"
                  disabled={!nTonnes || !nFrom || !nTo || savingNeed}
                  onClick={handleAddMaterialNeed}
                >
            ...
            materialNeeds.map((n) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold">
                      {MATERIAL_LABELS[n.materialCategory] ?? n.materialCategory}
                    </p>
                    <Badge variant="outline" className="text-[10px] h-4 px-1.5 rounded-full">
                      {n.estimatedTonnes} t
                    </Badge>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {fmtDate(n.neededFrom)} → {fmtDate(n.neededTo)}
                  </p>
                  {n.notes && <p className="text-xs text-muted-foreground truncate">{n.notes}</p>}
                </div>
                <button
                  onClick={() => handleDeleteMaterialNeed(n.id)}
                  className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-all"
                  title="Dzēst"
                >
                  <Trash2 className="h-3.5 w-3.5" />
