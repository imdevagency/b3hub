            ...
            {offer.supplier.rating != null && (
              <div className="flex items-center gap-1 mt-0.5">
                {[1, 2, 3, 4, 5].map((n) => (
                  <Star
                    key={n}
                    className={`size-3 ${n <= Math.round(offer.supplier.rating!) ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground/30 fill-muted-foreground/10'}`}
                  />
                ))}
...
export default function OrderPage() {
  ...
  useEffect(() => {
    if (!isLoading && user) {
      if (user.canSell && !user.canTransport) router.replace('/dashboard/supplier');
      else if (user.canTransport && !user.canSell && !user.isCompany)
        router.replace('/dashboard/transporter');
      else router.replace('/dashboard/order');
    }
  ...
  return (
    <>
      ...
      <div className="relative flex h-[calc(100vh-56px)] overflow-hidden">
        ...
        {!activeCategory && (
          <div className="relative z-10 m-auto w-full max-w-4xl px-4 py-8 overflow-y-auto max-h-full">
            <div className="mb-8 text-center">
              <h1 className="text-3xl font-extrabold text-gray-900 sm:text-4xl">
                Ko jums nepieciešams?
              </h1>
              <p className="mt-2 text-base text-gray-500">
                Izvēlieties materiālu kategoriju — cenas redzēsiet uzreiz
              </p>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {offers.length > 0 && step === 'offers' && (
                    <span className="font-bold text-lg text-primary shrink-0">
                      no €{[...offers].sort((a, b) => a.totalPrice - b.totalPrice)[0].totalPrice}
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Aprakstiet materiālu un norādiet daudzumu
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder="piem. Grants 0-32, Smiltis smalka..."
                      className="rounded-2xl h-13 bg-muted/40 border-0 px-4 text-[15px] font-medium focus:ring-2 focus:ring-foreground/10 transition-shadow"
                    />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-muted/40 hover:bg-muted/70 transition-colors"
                    >
                    ...
                    <Input
                      ...
                      className="rounded-2xl h-11 bg-muted/40 border-0 px-4 text-[15px] font-bold text-center focus:ring-2 focus:ring-foreground/10 transition-shadow"
                    />
                    <button
                      onClick={() => patch({ quantity: form.quantity + 1 })}
                      className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-muted/40 hover:bg-muted/70 transition-colors"
                    >
                    ...
                    <Select
                      value={form.unit}
                      onValueChange={(val) => patch({ unit: val as MaterialUnit })}
                    >
                      <SelectTrigger className="w-32 rounded-2xl h-11 bg-muted/40 border-0 text-[15px] font-medium focus:ring-2 focus:ring-foreground/10 transition-shadow shrink-0">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="rounded-xl">
                        {UNITS.map((u) => (
            ...
            {step === 'rfq-sent' && (
              <div className="flex flex-col items-center justify-center py-10 text-center space-y-5 animate-in zoom-in-95">
                ...
                <p className="text-sm text-muted-foreground max-w-xs">
                  Piegādātāji jūsu rajonā saņēma paziņojumu. Kad kāds atbildēs, jūs saņemsiet
                  e-pastu.
                </p>
                <Button
