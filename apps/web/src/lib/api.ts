...
export async function getFrameworkContract(id: string, token: string): Promise<ApiFrameworkContract> {
  return apiFetch<ApiFrameworkContract>(`/framework-contracts/${id}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
}

export async function createFrameworkContract(data: CreateFrameworkContractInput, token: string): Promise<ApiFrameworkContract> {
  return apiFetch<ApiFrameworkContract>('/framework-contracts', {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
