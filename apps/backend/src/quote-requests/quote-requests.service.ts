  ...
  async acceptResponse(requestId: string, responseId: string, userId: string) {
    ...
    const [, , order] = await this.prisma.$transaction(async (tx) => {
      ...
      const newOrder = await tx.order.create({
        data: {
          ...
          deliveryState: '-',
          deliveryPostal: '',
          deliveryLat: req.deliveryLat ?? undefined,
          deliveryLng: req.deliveryLng ?? undefined,
          subtotal,
          tax,
          deliveryFee: 0,
          total,
    ...
    this.notifications
      .create({
        ...
        title: 'Pasūtījums izveidots — nepieciešama apmaksa',
    ...
    // Invoice will be auto-generated by OrdersService.updateStatus(CONFIRMED)
    // when the seller confirms the order — no need to create it here.
