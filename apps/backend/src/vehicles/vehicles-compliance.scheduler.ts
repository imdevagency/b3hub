...
export class VehiclesComplianceScheduler {
  ...
  async handleExpiryCheck() {
    await withCronLock(
      this.prisma,
      'vehicles-compliance-expiry-check',
      async () => {
        ...
        if (expiring.length === 0) {
          this.logger.log(
            'No vehicles with expiring compliance — nothing to do',
          );
          return;
        }
        ...
        for (const v of expiring) {
          ...
          const checks: Array<{
            field: 'insuranceExpiry' | 'inspectionExpiry';
            date: Date | null;
            name: string;
          }> = [
            {
            ...
            },
            {
              ...
              name: 'Tehniskā apskate',
            },
          ];

          for (const check of checks) {
            if (!check.date || check.date < now || check.date > in30Days)
              continue;
