 ...
 * seed-sandbox.ts
 *
 * Populates a full sandbox environment on top of the base seed (seed.ts).
 * Run AFTER `npm run seed` — this script expects demo users & companies to exist.
 *
 * What it seeds:
 *  - Carrier company (RīgaKargo SIA) with CarrierPricing + ServiceZones
 *  - Vehicles for the carrier
 *  - Orders at every lifecycle stage (PENDING → COMPLETED + CANCELLED)
 *  - SkipHire orders at every stage
 *  - QuoteRequests with supplier responses
 *  - A FrameworkContract with positions
 *  - Invoices for completed/confirmed orders
 *  - Notifications for each demo user
 *
 * Usage:
 *   cd apps/backend
 *   npm run seed            # base users + companies + materials
 *   npm run seed:sandbox    # this file
 */
...
// Use direct URL when available (bypasses PgBouncer which blocks prepared statements)
...
async function getCompany(registrationNum: string) {
  const c = await prisma.company.findUnique({ where: { registrationNum } });
  if (!c) throw new Error(`Company ${registrationNum} not found — run "npm run seed" first.`);
  return c;
}

async function getMaterial(name: string, supplierId: string) {
...
async function main() {
  ...
  const buyerCompany = await getCompany('LV40001234567'); // RīgasBūve SIA
  const supplierCompany = await getCompany('LV40003123456'); // BaltSmiltis SIA
  const baltbuveCompany = await getCompany('LV40009999999'); // BaltBūve SIA
  ...
  // Link driver@demo.com to carrier company
  if (!driver.companyId) {
  ...
  const vehicles = [
    ...
    },
    {
      ...
      vehicleType: VehicleType.SKIP_LOADER,
      capacity: 18,
      status: VehicleStatus.ACTIVE,
    },
    {
  ...
  // Find materials from BaltSmiltis
  ...
  // ── 6. Transport jobs linked to orders ────────────────────────────────────
  console.log('\n🚛  Seeding transport jobs...');

  const jobStatuses: { orderNum: string; jobNum: string; status: TransportJobStatus }[] = [
    ...
    const fc = await prisma.frameworkContract.create({
      data: {
        ...
        positions: {
          create: [
            {
              positionType: FrameworkPositionType.MATERIAL_DELIVERY,
  ...
  const invoiceOrders = createdOrders.filter((o) =>
    ['COMPLETED', 'CONFIRMED', 'IN_PROGRESS'].includes(o.status),
  );
