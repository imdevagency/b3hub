...
| **Skip Hire — Ordering** *(removed from scope)* | Out of scope | Out of scope | Out of scope | Skip hire removed from Bilt scope. |
| **Skip Hire — Driver (skips)** *(removed from scope)* | Out of scope | Out of scope | Out of scope | Skip hire removed from Bilt scope. |
| **Containers** *(removed from scope)* | Out of scope | Out of scope | Out of scope | Containers removed from Bilt scope. |
...
| **Admin — Dashboard**            | ✅ `admin/`                             | ✅ `/dashboard/admin` (GMV KPI cards + monthly trend charts + revenue breakdown)                                                                                                                                              | 📵                                                                                     | Single admin scope managing all 4 market sides. Live badge counts every 30 s. Deploy with `NEXT_PUBLIC_APP_MODE=admin` for admin-only Vercel instance.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| **Admin — Skip Hire**            | ✅ `skip-hire/`                         | ✅ `/dashboard/admin/skip-hire`                                                                                                                                                                                               | 📵                                                                                     |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |     | **Admin — Skip Size Catalogue** | ✅ `admin/` (via `SkipSizeDefinition`) | ✅ `/dashboard/admin/skip-sizes` | 📵  | CMS-managed. Replaces hardcoded `SkipSize` enum. `SkipCategory` enum: `SKIP \| BIG_BAG \| CONTAINER`. 8 sizes seeded. Mobile wizard fetches sizes dynamically via `GET /skip-hire/sizes`. |     | **Admin — Payments** | ✅ `payments/` | ✅ `/dashboard/admin/payments` | 📵  | Release / refund individual payments |
...
| **Admin — B3 Fields**            | ✅ `b3-fields/`                         | ✅ `/dashboard/admin/b3-fields`                                                                                                                                                                                               | 📵                                                                                     | B3 Field locations. Fields modelled as `RecyclingCenter` with `licensed` flag (licensed fields can process and certify construction waste).                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Admin — Recycling Portal**     | ✅ `admin/` (b3-recycling endpoints)    | ✅ `/dashboard/b3-recycling` — overview, jobs, waste-log, certificates, apus                                                                                                                                                  | 📵                                                                                     | Admin portal for managing licensed recycling facility operations: inbound jobs, waste records, certificates, APUS reporting.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **SkipSizeDefinition migration** — `SkipSize` enum removed from schema, replaced by `SkipSizeDefinition` model + `SkipCategory` enum (`SKIP\|BIG_BAG\|CONTAINER`). `SkipHireOrder.skipSize` and `CarrierPricing.skipSize` changed to `String`. All backend/mobile/admin references updated. 8 sizes seeded. Admin CMS at `/dashboard/admin/skip-sizes`. Mobile wizard fetches sizes dynamically; falls back to hardcoded list. | ✅     | Backend + Web + Mobile |
| **`@/components/ui/table`** — shadcn-style `Table`/`TableHeader`/`TableBody`/`TableRow`/`TableHead`/`TableCell`/`TableFooter`/`TableCaption` component created. Fixes `fee-config`, `framework-contracts`, `invoices`, `payouts` admin pages that imported it. `company.type` → `company.companyType` type mismatch in `fee-config` fixed. Web `tsc --noEmit` now passes with zero errors.                                     | ✅     | Web/Infrastructure     |
...
### April 29 2026 — Admin ERP + performance fixes

| Item                                                                                                                                                                                                                                                                                         | Status | Impact               |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | -------------------- |
| **Admin dashboard — 3 new widgets**: "Nepieciešama rīcība" pending-actions inbox (applications, disputes, support, payout queue, expiring docs); "Pasūtījumu konveijers" color-coded order pipeline bar chart; "Šodienas piegādes" today's delivery schedule table with buyer + driver names | ✅     | Admin/Operations     |
| **Admin documents page** — `/dashboard/admin/documents`: stats strip, search + type/status/source filters, paginated table with archive/restore dialog; sidebar link added; backend `GET /admin/documents` + `PATCH /admin/documents/:id/status` endpoints wired end-to-end                  | ✅     | Admin/Documents      |
| **Backend: N+1 eliminated in `generateCommissionInvoices`** — replaced N individual `findFirst` idempotency checks with a single batched `findMany` + `Set` lookup; O(1) regardless of supplier count                                                                                        | ✅     | Backend/Performance  |
| **Backend: parallel agent-supplier invoice creation** — replaced sequential `for...await` loop in `invoices.service.ts` with `Promise.all`; all per-supplier invoices now insert concurrently                                                                                                | ✅     | Backend/Performance  |
| **Backend: silent `.catch(() => [])` in Paysera webhook** — carrier user lookup failure is now logged as `warn` before returning the empty fallback; failure no longer swallowed silently                                                                                                    | ✅     | Backend/Reliability  |
| **Sentry on mobile** — confirmed fully initialized: `Sentry.init()` at module level + `Sentry.wrap()` on root layout in `app/_layout.tsx`; `@sentry/react-native` installed; `enabled: false` in dev, 20% trace sample in prod                                                               | ✅     | Mobile/Observability |
| **Health endpoint** — `GET /` returns `{ status: 'ok', version: '1' }` (not "Hello World!"); Railway health checks pass                                                                                                                                                                      | ✅     | Backend/Ops          |
...
| Verify Paysera project credentials + native EAS build for mobile checkout web-view                                                                                                                                       | Runtime readiness — **hard blocker before launch**                                                                                                                                                                                                                                                           | Web + mobile           |
...
| ~~In-app support contact — no way to reach Bilt from inside the app if something goes wrong~~                                                                                                                           | ~~User trust / support~~ — **DONE**: `SupportThread`/`SupportMessage` models + `/api/v1/support` endpoints + `support-chat.tsx` screen                                                                                                                                                                       | ~~Mobile + Web~~       |
...
- dashboard/(internal)/b3-recycling/finances
...
- dashboard/(platform)/company/integrations
...
- dashboard/(platform)/recycling/apus
- dashboard/(platform)/recycling/certificates
